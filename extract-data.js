...
(async function(shortnames) {
  ...
  shortnames.forEach(async (shortname) =>  {
    const trShortname = shortname.tr ?? shortname;
    const wptShortname = shortname.wpt ?? shortname;
    const idlData = (await fetch("https://w3c.github.io/webref/ed/idlparsed/" + trShortname + ".json").then(r => r.json())).idlparsed;
    ...
    let impldata = [];
    if (!testPath) {
      interfaces.error = `No idlharness tests found for ${trShortname}`;
    } else {
    ...
    if (!interfaceNames.length) {
      interfaces.error = `No interfaces found in ${trShortname}`;
    }
    ...
    await fs.writeFile(trShortname + ".json", JSON.stringify( interfaces, null, 2));
  });
